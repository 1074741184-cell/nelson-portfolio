...
class NelsonBatchStitcher:
    def __init__(self, main_root: tk.Tk):
        ...
        self.root.geometry("800x900")  # 调整默认大小，更适合大多数屏幕
        self.root.minsize(620, 700)  # 设置最小窗口大小
    ...
    def setup_ui(self):
        ...
        # 创建滚动容器
        scroll_container = tk.Frame(self.root, bg=self.bg_dark)
        scroll_container.pack(fill="both", expand=True)

        # 创建Canvas和Scrollbar
        self.canvas = tk.Canvas(scroll_container, bg=self.bg_dark, highlightthickness=0)
        scrollbar = tk.Scrollbar(scroll_container, orient="vertical", command=self.canvas.yview)
        scrollable_frame = tk.Frame(self.canvas, bg=self.bg_dark)
        ...
        def _configure_scrollable_frame(event):
            self.canvas.itemconfig(self.canvas.create_window((0, 0), window=scrollable_frame, anchor="nw"), width=event.width)
        self.canvas.bind('<Configure>', _configure_scrollable_frame)
        ...
        # 主内容容器现在是scrollable_frame
        main_container = tk.Frame(scrollable_frame, bg=self.bg_dark, padx=25, pady=15)
        ...
        self.sub_entry.insert(0, "Nelson测试");
        ...
        self.run_btn = tk.Button(self.root, text="一键执行全自动化剪辑", command=self.start_thread, bg=self.theme_cyan,
                                 fg="#000000", font=("Microsoft YaHei", 11, "bold"), bd=0, height=2, state="disabled")
        self.run_btn.pack(fill="x", side="bottom")
    ...
    def get_duration(self, file_path):
        try:
            cmd = [resource_path('ffprobe.exe'), '-v', 'quiet', '-print_format', 'json', '-show_format', file_path]
        ...
        try:
            ...
            for index, name_a in enumerate(files_a):
                ...
                try:
                    ...
                    cmd = [resource_path('ffmpeg.exe'), '-y', '-hwaccel', 'auto', '-i', in_a, '-i', in_b, '-i', in_v,
